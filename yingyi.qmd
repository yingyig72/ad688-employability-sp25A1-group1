...
count_df = pd.Series({False: 4465, True: 3014}, name='AI_JOB')
count_df.index.name = 'AI_JOB'
display(count_df.to_frame('count'))
```
...
proportion_df = pd.Series(data)
proportion_df.index.names = ['AREA_LABEL', 'SKILL_LEVEL']
display(proportion_df.to_frame('proportion'))
```
...
coef_df = pd.DataFrame({
    'Feature': ['AI_JOB', 'URBAN_BINARY', 'MIN_YEARS_EXPERIENCE', 'AI_URBAN_INTERACTION', 'SKILL_LEVEL_Medium', 'SKILL_LEVEL_High', 'AI_SKILL_INTERACTION'],
    'Coefficient': [0.079960, 0.150160, 0.046561, -0.095228, 0.113236, 0.263699, -0.010044]
})
display(coef_df)
```