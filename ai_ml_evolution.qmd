...
for col in ['SKILLS_NAME', 'SPECIALIZED_SKILLS_NAME', 'SOFTWARE_SKILLS_NAME']:
    df[col + '_list'] = df[col].apply(parse_json)

# Combine all skills into one list per posting
df['all_skills'] = df.apply(
    lambda r: r['SKILLS_NAME_list'] + r['SPECIALIZED_SKILLS_NAME_list'] + r['SOFTWARE_SKILLS_NAME_list'],
    axis=1
)
...
monthly['posted_month_str'] = monthly['posted_month'].astype(str)

print(f"{'Month':<12} {'Total':>8} {'AI/ML':>8} {'% AI/ML':>8}")
print(f"{'-'*40}")
for _, row in monthly.iterrows():
    print(f"{row['posted_month_str']:<12} {row['total']:>8,} {row['ai_ml']:>8,} {row['pct_ai_ml']:>7.1f}%")
```
...
ax1.set_xticks(x)
ax1.set_xticklabels(monthly['posted_month_str'], rotation=45, ha='right', fontsize=9)
...
ax2.set_xticks(x)
ax2.set_xticklabels(monthly['posted_month_str'], rotation=45, ha='right', fontsize=9)
...
```{python}
# Top 20 job titles (TITLE_NAME) with AI/ML requirements
...
for label, subset in [("AI/ML Postings", df[df['has_ai_ml']]),
    ...
    edu = subset['MIN_EDULEVELS_NAME'].apply(parse_json).explode().dropna()
    edu = edu[edu != '']
    edu_counts = edu.value_counts().head(8)
