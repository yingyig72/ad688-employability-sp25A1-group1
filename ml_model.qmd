...
The chart below shows the features with the largest absolute coefficients. Positive coefficients (green) push a posting toward "requires AI/ML"; negative coefficients (red) push it away. **Hover over any bar** for the exact log-odds value and the implied multiplicative effect on the odds.

```{python}
#| label: fig-coefficients
#| fig-cap: "Strongest predictors of AI/ML skill requirements (top 20 by absolute coefficient). Hover for details."

coefs = pd.Series(model.coef_[0], index=X.columns).sort_values()

# Show top 10 negative + top 10 positive, sorted ascending so largest positive is on top
...
def prettify(c):
    return (
        c.replace("industry_", "Industry: ")
         .replace("role_category_", "Role: ")
         .replace("education_", "Education: ")
         .replace("has_", "Skill: ")
         .replace("_", " ")
         .title()
         .replace("Industry:", "Industry:")  # title() preservation
    )

labels = [prettify(c) for c in top.index]
values = top.values
...
hover = [
    f"<b>{lbl}</b><br>"
    f"Log-odds coefficient: {v:+.3f}<br>"
    f"Odds multiplier: ×{o:.2f}<br>"
    f"Direction: {'Increases' if v > 0 else 'Decreases'} AI/ML probability"
    for lbl, v, o in zip(labels, values, odds_ratios)
]
...
fig.update_layout(
    title=dict(
        text="<b>Strongest predictors of AI/ML skill requirements</b>",
        font=dict(size=15),
        x=0.02,
    ),
    xaxis=dict(
